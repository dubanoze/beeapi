...
def get_access(num):
	...
	log,passw=session.query(acc.login,acc.password).filter(acc.operator_agree==n.operator_agree).filter(acc.access_type==1).one()
	return log,passw
...
def available_all():
	...
	ctn_list=session.query(ctn.msisdn,ctn.operator_agree).filter(ctn.operator_agree.in_(oan_list),ctn.status==1).group_by(ctn.operator_tarif).all()
	print('получили список номеров')
	for el in ctn_list:
		l_a=None
		query=session.query(acc.login,acc.password).filter(acc.operator_agree==el.operator_agree).filter(acc.access_type==1)
		try:
			log,passw=query.one()
		except:
			try:
...
def availables():
	...
	for number in file:
		pre_rez=[]
		ctn_n=session.query(ctn.msisdn,ctn.operator_agree,ctn.operator_tarif,ctn.status).filter(ctn.msisdn==number.rstrip()).one()
		ctn_tp=session.query(btp.name).filter(btp.i_id==ctn_n.operator_tarif).one()
		query=session.query(acc.login,acc.password).filter(acc.operator_agree==ctn_n.operator_agree).filter(acc.access_type==1)
		log,passw=query.one()
	...
	return rez
	ex_write(names=['','','','','','','','','','','','','','','',''],values=rez,path='C:/Users/админ/Desktop/рез.xlsx')
...
def get_tarifs():
	...
	ctns=session.query(ctn.msisdn,ctn.operator_tarif,ctn.operator_agree).filter(ctn.operator_agree.in_(agrs)).group_by(ctn.operator_tarif).all()
	out=[]
	errors=[]
	for phone in ctns:

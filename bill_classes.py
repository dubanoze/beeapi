...
class object(Base):
	...
	properties_t=relationship('properties',backref='a_properties',primaryjoin="object.object_id==properties.object_id")
...
class properties(Base):
	...
	unique=Column(TINYINT(unsigned=True),nullable=False) #Признак того, что значение свойства должно быть уникальным для каждого экземпляра класса.
	input=Column(ENUM('text','number',
					  'date','select',
					  'autocomplete',
					  'textarea','password'))
	context_search=Column(TINYINT,nullable=False) #Учавствует в контекстном поиске по ссылке из другого класса. Значение >0 носит признак сортировки.
	sort=Column(TINYINT(unsigned=True),nullable=False)
	ungapped=Column(TINYINT(unsigned=True),nullable=False)
	description=Column(VARCHAR(length=250))
	values_to_select=Column(VARCHAR(length=128)) #Список возможных вариантов значений для выбора в интерфейсе на основе <select>. Формат: Число:Имя
	...
	key=Column(TINYINT(unsigned=True),nullable=False) #Признак того, что свойство является ключевым в классе типа "связь"


	object=relationship('object',uselist=False,backref=backref('a_object'),foreign_keys=ref_object,primaryjoin="object.object_id==properties.ref_object")

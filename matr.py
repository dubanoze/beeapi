from beeclient.bill_classes import ClassGetter, session as eko_ses
...
def check_values(record):
    ...
    if p_type in [1, 2, 3]:
        ...
        try:
            ...
            logging.warning("Incorrect value {}, record:{}".format(record['tariff_mobile_packet'], record))
...
def main(p=1):
    if p == 1:
        with open('C:/Users/админ/Desktop/vals.pickle', 'rb') as file:
        ...
        with open('C:/Users/админ/Desktop/vals.pickle', 'wb') as file:
    ...
    for rec in vals:
        if rec['object_type'] == "тариф корпорации":
            check_values(rec)
            if vals.index(rec) % 100 == 0:
                logging.info('Ready {} of {} records'.format(vals.index(rec)+1, len(vals)))

...
def getCTNInfoList(*args):
    ...
    if level == 'ctn':
        ...
        for el in rez:
            els.append([el['ctn'][1:], el['statusDate'], el['status'], el['activationDate'], el['pricePlan'],
                        el['reasonStatus'], el['subscriberHLR'], el['lastActivity']])
    return els
...
def choice(args):
    try:
        ...
        if args[1] == 'ctn_down':
            ...
            names = ['Номер', 'Дата изменения статуса', 'Статус', "Дата активации", 'Тариф',
                     'Причина изменения статуса', "HLR", "Последняя активность"]
...
def paym(*args):
    ...
    for el in [147807239,
        ...
        rez = api.getPaymentList(level='ban', bday=int(date[0]), bmonth=int(date[1]), eday=int(date[0]),
                                 emonth=int(date[1]))
...
def mass_services():
    ...
    for row in xl.rows:
        ...
        q += 1
    print(rezult_excel)
    ex_write(['ctn', 'service', 'descr', 'status', 'tarif', 'action'], values=rezult_excel,
             path=d + '\services_out.xlsx')
    conn.close()
...
def get():
    for ban in [370174062, 429313574, 434332598, 434573783, 434573954, 434574188, 434574645]:
        ...
        for per in ["2014-05-31", "2014-06-30", "2014-07-31", "2014-08-31"]:
            ...
            ex_write(names=rez[0].__keylist__, values=rez,
                     path='C:/Users/админ/Desktop/табло/{}_{}.xlsx'.format(ban, per))
...
def checkPP():
    ...
    for phone in ctns:
        ...
        for pp in rez:
            rezult.append(
                [c_pp['name'], c_pp['entityName'], pp['name'], pp['entityName'], pp['rcRate'], pp['chargeAmount']])
